...
@Service
public class WeeklyDataBulkLoader {

    private static final int BATCH = 10_000;
    private static final int COLUMN_COUNT = 25;

    private static final Logger log = LoggerFactory.getLogger(WeeklyDataBulkLoader.class);
    ...
    public WeeklyDataLoadResult bulkLoad(String filePath) {
        ...
        if (!result.success()) {
            return WeeklyDataLoadResult.error(
                    loadSessionId,
                    result.message()
            );
        }
        ...
        if (filePath == null || filePath.isBlank()) {
            ...
            return WeeklyDataLoadResult.error(loadSessionId, "filePath is empty");
        }
        ...
        if (!Files.exists(path)) {
            ...
            return WeeklyDataLoadResult.error(loadSessionId, "file does not exist: " + filePath);
        }
        ...
        if (!Files.isReadable(path)) {
            ...
            return WeeklyDataLoadResult.error(loadSessionId, "file is not readable: " + filePath);
        }
        ...
        if (!lower.endsWith(".xlsx")) {
            ...
            return WeeklyDataLoadResult.error(loadSessionId, "Only Excel files (.xlsx) are allowed");
        }
...
/////////////////////////////////////////////////
        ...
        String sql = """
                INSERT INTO dbo.Weekly_data_raw
                (LoadSessionId, Year21, Week21, YearCorr, WeekCorr, Year, Week,
                 SalesChannelBpo, StoreRusBpo, StoreRus,
                 MfpDivisionNew, MfpDepartment,
                 SkuSeasonBudget, TypeOfSales,
                 TotalStockPcs, TotalStockDdp,
                 SalesPcs, SalesRub,
                 Revenue, Gp, DiscountTotalRub,
                 MfpDivision, Season, Month, Bundle, Seasonality)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """;
        ...
        try {
            ...
            try (PreparedStatement ps = c.prepareStatement(sql);
                ...
                if (!sheets.hasNext()) {
                    ...
                    return WeeklyDataLoadResult.error(loadSessionId,"Excel file does not contain sheets");
                }
            ...
            return WeeklyDataLoadResult.error(loadSessionId, "SQL error after processing " + stagedRows[0] + " rows: " + ex.getMessage());
        } catch (Exception ex) {
            if (c != null) {
                try {
                    c.rollback();
            ...
            return WeeklyDataLoadResult.error(loadSessionId, "Fatal error after processing " + stagedRows[0] + " rows: " + ex.getMessage());
        }
        finally {
            if (c != null) {
                try {
        ...
        return WeeklyDataLoadResult.ok(loadSessionId);
    }
    ...
    private static int dbSize(int idx) {
        return switch (idx) {
            case 0, 1, 2, 3, 4, 5 -> 50;
            case 6, 7, 8, 9, 10, 20 -> 255;
            case 11, 12, 21, 22, 23, 24 -> 100;
            case 13, 14, 15, 16, 17, 18, 19 -> 50;
            default -> 255;
        };
    }
...
///////////////////////////////////////////////////////
    ...
    private MethodSessionResult createLoadSession(String filePath) {
        ...
        try {
            ...
            INSERT INTO dbo.Weekly_data_Load_session (FileName, FilePath, Status)
            OUTPUT INSERTED.Id
            VALUES (?, ?, 'RUNNING')
            """)) {
    ...
    private MethodSessionResult finishLoadSession(Long loadSessionId, String status, String message) {
        ...
        try {
            ...
            try (PreparedStatement ps = c.prepareStatement("""
            UPDATE dbo.Weekly_data_Load_session
